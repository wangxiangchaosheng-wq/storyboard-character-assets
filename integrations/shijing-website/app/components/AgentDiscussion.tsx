...
export default function AgentDiscussion(){
  ...
  return <main className="scene-viewport discussion-fullwidth" ref={viewport}>
    ...
    <div className="artboard" style={{top:`max(0px, calc((100svh - ${941*scale}px) / 2))`,transform:`translateX(-50%) scale(${scale})`,transformOrigin:"top center"}}><OriginalScene assets={assets} activeId={activeSlot} pristine={false} editingInput={true}/><CandleHome onStrategy={()=>setStrategyOpen(true)} onEvents={()=>setEventsOpen(true)}/>
      ...
      <section className="dialogue-panel" aria-label="廷议"><div className="conversation" role="log" aria-live="polite" ref={log}>{view?.messages.length?view.messages.map(m=><article className={`message ${m.kind==='player'?'user':'assistant'}`} key={m.id}><span className="user-mark">{(m.name||'史官').slice(0,1)}</span><div className="message-content"><b>{m.name||'史官'}</b><p>{m.text}</p></div></article>):<article className="message assistant"><div className="message-content"><b>史官</b><p>{view?.spec.scenario.background||'正在准备本次议题…'}</p><p>{view?.strategy?.localSimulation?'本地推演已就绪。勾选下达行动后，可发送“魏延 行军 长安”“魏延 补给 汉中 10000”或“推进 1 天”。数值为带来源说明的模拟设定。':view?.mode==='standalone'?'已有的人物图会从素材库载入。角色讨论与行动裁决需连接队友推演引擎后开启。':''}</p></div></article>}</div></section>
      <form className="composer" onSubmit={e=>{e.preventDefault();void submit();}}><label className="sr-only" htmlFor="agent-message">输入讨论或行动</label><textarea id="agent-message" rows={1} maxLength={2000} value={text} onChange={e=>setText(e.target.value)} disabled={sending||!(view?.mode==='engine'||view?.strategy?.localSimulation)}/><button type="submit" className="art-button send-button" aria-label="发送" disabled={sending||!text.trim()||!(view?.mode==='engine'||view?.strategy?.localSimulation)}/></form>
      {!assets.storyboard.image&&<div className="story-pending"><strong>故事板</strong><p>{storyProgress}</p></div>}
    </div>
    {(view?.mode==='engine'||view?.strategy?.localSimulation)&&<label className="agent-action"><input type="checkbox" checked={act} onChange={e=>setAct(e.target.checked)}/>下达行动（由裁判结算并更新世界）；不勾选时只讨论</label>}

...
export class Store {
  ...
  retry(id:string){const j=this.job(id);assert(['failed','interrupted'].includes(j.status),'只有失败或中断任务可重试',409);j.error=undefined;if(j.kind==='portrait'&&j.draftAttempt!==undefined&&j.plan&&existsSync(this.draftPath(j))){j.resumeDraft=true;j.feedback='';}if(j.kind==='portrait'&&!j.transparency&&j.trace.some(t=>t.detail.includes('[ALPHA]')))j.transparency='chroma';this.step(j,'queued','用户重试');return j;}

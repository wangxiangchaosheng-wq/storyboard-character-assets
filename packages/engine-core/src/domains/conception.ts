...
export function specFromJson(raw: string, domain: Domain): ScenarioSpec | null {
  ...
  const scenario: Scenario = {
    title: obj.scenario.title ?? obj.title ?? '未命名推演',
    background: obj.scenario.background ?? '',
    conflict: obj.scenario.conflict ?? '',
    participants: obj.scenario.participants ?? obj.cast.map((c: any) => c.id),
    rounds: obj.scenario.rounds ?? 3,
    decisionPoint: obj.scenario.decisionPoint ?? '你做出怎样的决断？',
    successCriteria: obj.scenario.successCriteria ?? '',
  };
  const metrics: MetricDef[] = obj.metrics.map((m: any) => ({
    key: String(m.key),
  ...
  const rules: RuleDef[] = (obj.rules ?? []).map((r: any) => ({
  ...
  const cast: Persona[] = obj.cast.map((c: any) => ({
    id: String(c.id),
    ...
    prompt: String(c.prompt ?? `你是${c.name}，立场${c.stance}。`),
    traits: {
      competence: Number(c.traits?.competence ?? 60),
      loyalty: Number(c.traits?.loyalty ?? 60),
      ambition: Number(c.traits?.ambition ?? 50),
      power: Number(c.traits?.power ?? 50),
    },
  }));
  return { domain, title: obj.title ?? scenario.title, scenario, cast, metrics, rules, rounds: scenario.rounds };
}

  ...
  for (const tk of toks) {
    if (tk.t === 'num' || tk.t === 'id') out.push(tk);
      ...
      while (
        ...
        (RIGHT.has(tk.v) ? PREC[(stack[stack.length - 1] as { t: 'op'; v: string }).v] > PREC[tk.v]
                          : PREC[(stack[stack.length - 1] as { t: 'op'; v: string }).v] >= PREC[tk.v])
      ) out.push(stack.pop()!);
      stack.push(tk);
    } else if (tk.t === 'comma') {
...
export function applyDrift(state: State, rules: RuleDef[], _metrics: MetricDef[], round?: number): Delta[] {
  const out: Delta[] = [];
  // 把当前回合数注入公式求值上下文（key 'r'），让 drift 公式可以依赖回合做周期性振荡
  const formulaState: State = round !== undefined ? { ...state, r: round } : state;
  for (const r of rules) {
    if (r.kind !== 'drift') continue;
    if (r.effect) {
      const d = r.effect(formulaState);
      for (const [k, v] of Object.entries(d)) {
        out.push({ key: k, amount: v, reason: r.label, source: r.id });
      }
    } else if (r.target && r.formula != null) {
      const v = evalFormula(r.formula, formulaState);

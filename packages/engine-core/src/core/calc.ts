...
function parseDeltas(raw: string, rules: RuleDef[], metrics: MetricDef[]): Delta[] {
  ...
  if (Array.isArray(j.deltas)) {
    for (const d of j.deltas) if (d && typeof (d as J).key === 'string') list.push(d as { key: string; amount: number; reason?: string });
  } else if (typeof j === 'object' && j !== null) {
    for (const [k, v] of Object.entries(j)) {
      if (k === 'deltas') continue;
      if (typeof v === 'number') list.push({ key: k, amount: v });
      else if (v && typeof (v as J).amount === 'number')
        list.push({ key: k, amount: (v as J).amount as number, reason: (v as J).reason as string | undefined });
    }
  }
...
function mockReaction(
  ...
  for (const r of rules) {
    ...
    for (const key of r.appliesTo ?? []) {
      ...
      const h = hash(key + '|' + decision.intent + '|' + cast.length + '|' + (dir === 'positive' ? 1 : dir === 'negative' ? -1 : 0));
      ...
      // 意图方向加成：正向意图 → 好方向指标更易正向；负向意图 → 反方向
      if (dir === 'positive' && goodDir) amount = Math.abs(amount);
      else if (dir === 'negative' && !goodDir) amount = -Math.abs(amount);
      // 生成更有叙事感的 reason
      let reason = r.label + '（启发式）';
      if (amount > 0) reason = r.label + `：意图「${decision.intent?.slice(0, 12)}」推动 ${m.label} 上升`;
      else if (amount < 0) reason = r.label + `：意图「${decision.intent?.slice(0, 12)}」导致 ${m.label} 下降`;
      out.push({ key, amount, reason, source: r.id });
    }
  }
  return out;
}
...
export async function computeSettlement(
  ...
  const decay = await applyDecay(decision, cast, undefined as unknown as MemoryBank, llm);

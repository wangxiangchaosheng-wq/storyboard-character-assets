...
export function registerGameRoutes(app: FastifyInstance, deps: GamesDeps): void {
  ...
  app.post('/api/games', async (req, reply) => {
    ...
    const rt = await startSession(store, llm, spec, body.specId, gameId, body.player);
    putGame(store, {
      id: gameId,
      spec_id: body.specId,
      title: spec.title,
  ...
  app.post('/api/games/:id/message', async (req, reply) => {
    ...
    const { rt, added } = await handleMessage(store, llm, spec, row.spec_id, id, body);
    putGame(store, {
      id,
      spec_id: row.spec_id,
      title: spec.title,
